...
async function encodeRequest(request: EncodeRequest): Promise<EncodeResponse> {
    ...
    const result = await encodings.reduce(async (contentPromise, nextEncoding) => {
        const content = await contentPromise;
        return await encodeBuffer(content, nextEncoding);
    }, Promise.resolve(Buffer.from(buffer) as Buffer<ArrayBufferLike>));

    return {
        id,
        encodedBuffer: bufferToArrayBuffer(result as Buffer<ArrayBufferLike>)
    };
}
